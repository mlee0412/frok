...
export async function POST(req: NextRequest, context: { params: Promise<{ threadId: string }> }) {
  ...
  try {
      ...
      messages = [
        {
          role: 'system',
          content:
            'Generate a concise, descriptive title (3-6 words) for this conversation. Analyze the entire conversation to understand the main topic. Return ONLY the title, no quotes or extra text.',
        },
        ...(conversationHistory.slice(0, 5) as Array<{
          role: 'user' | 'assistant';
          content: string;
        }>), // Use first 5 messages
      ];
      ...
      messages = [
        {
          role: 'system',
          content:
            'Generate a concise, descriptive title (3-6 words) for a conversation based on the first user message. Return ONLY the title, no quotes or extra text.',
        },
        {
          role: 'user',
    ...
    return NextResponse.json({ ok: false, error: formatErrorMessage(error) }, { status: 500 });
  }
}
