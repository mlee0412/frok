...
function parseCSV(text: string): ParsedItem[] {
  ...
  // Naive CSV parser with quotes support
...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          detail: 'Send text/csv or application/json',
        },
        { status: 415 }
      );
    }
    ...
    // Process items into transaction rows
    ...
    for (const it of items) {
      ...
      const amountFixed = Number.isFinite(it.amount)
    ...
    const uniqueByKey = new Map<string, TransactionRow>();
    for (const r of rows) {
      if (!uniqueByKey.has(r.dedup_key)) {
        uniqueByKey.set(r.dedup_key, r);
      }
    }
    ...
    const existingKeys = new Set(
      (existingRows || []).map((r: { dedup_key: string }) => r.dedup_key)
    );
